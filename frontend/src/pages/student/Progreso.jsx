...
export default function Progreso() {
  ...
  const progresoPorMateria = inscripciones.map((insc, i) => {
    ...
    const COLS = ['bg-[rgba(124,58,237,0.12)]0', 'bg-[rgba(59,130,246,0.1)]0', 'bg-[rgba(16,185,129,0.1)]0', 'bg-[rgba(245,158,11,0.1)]0']
    const TXTS = ['text-purple-700', 'text-[#60A5FA]', 'text-[#34D399]', 'text-[#FBBF24]']
    ...
    <Layout rol="ESTUDIANTE" navItems={NAV}>
      <div className="max-w-6xl mx-auto px-5 py-6 space-y-6">
        <div style={{ background: "rgba(28,21,53,0.8)", backdropFilter: "blur(8px)" }} className=" px-4 py-2 rounded-xl inline-block">
          <h2 className="text-2xl font-black text-[#F3F4F6]">Mi Progreso </h2>
          <p className="text-[#E5E7EB] text-base mt-0.5">Analiza tu rendimiento academico</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div style={{ background: "#1C1535", borderRadius: 16, border: "1px solid rgba(124,58,237,0.18)" }} className="p-6 shadow-none flex flex-col items-center">
            <h3 className="font-bold text-[#E5E7EB] mb-4 self-start">Progreso General</h3>
            <div className="relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="text-xs text-[rgba(156,163,175,0.5)]">Completado</span>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 w-full mt-4">
              <div className="bg-[rgba(124,58,237,0.12)] border border-[rgba(124,58,237,0.3)] rounded-xl p-3 text-center">
                <p className="text-purple-700 font-bold">{entregadas}</p>
                <p className="text-xs text-[rgba(156,163,175,0.7)]">Entregadas</p>
              </div>
              <div className="bg-[rgba(59,130,246,0.1)] border border-[rgba(59,130,246,0.3)] rounded-xl p-3 text-center">
                <p className="text-[#60A5FA] font-bold">{total}</p>
                <p className="text-xs text-[rgba(156,163,175,0.7)]">Total</p>
              </div>
            </div>
          </div>

          <div style={{ background: "#1C1535", borderRadius: 16, border: "1px solid rgba(124,58,237,0.18)" }} className="p-6 shadow-none">
            <h3 className="font-bold text-[#E5E7EB] mb-4">Distribucion de Notas</h3>
            {rendimientoData.length === 0 ? (
              <div className="flex items-center justify-center h-40 text-[rgba(156,163,175,0.5)] text-sm">Sin calificaciones aún</div>
            ) : (
              <>
                ...
                <div className="space-y-2 mt-2">
                  {rendimientoData.map(item => (
                    <div key={item.name} className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-semibold text-[#E5E7EB]">{item.value}%</span>
                    </div>
                  ))}
                </div>
              </>
        ...
        {progresoPorMateria.length > 0 && (
          <div style={{ background: "#1C1535", borderRadius: 16, border: "1px solid rgba(124,58,237,0.18)" }} className="p-6 shadow-none">
            <h3 className="font-bold text-[#E5E7EB] mb-4">Progreso por Materia</h3>
            <div className="space-y-4">
              {progresoPorMateria.map(({ insc, progM, promM, entregadasM, total, col, txt }) => (
                <div key={insc.id} className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl flex-shrink-0 bg-[rgba(124,58,237,0.12)] border border-[rgba(124,58,237,0.3)]">
                    {insc.materia?.icono || '📖'}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm font-semibold text-[#E5E7EB]">{insc.materiaName}</span>
                    ...
                    <p className="text-xs text-[rgba(156,163,175,0.5)] mt-1">{entregadasM}/{total} actividades entregadas</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div style={{ background: "#1C1535", borderRadius: 16, border: "1px solid rgba(124,58,237,0.18)" }} className="p-6 shadow-none">
          <h3 className="font-bold text-[#E5E7EB] mb-4">Logros Obtenidos </h3>
          <div className="grid grid-cols-4 md:grid-cols-6 gap-3">
            {logros.map(logro => (
              <div key={logro.label}
                className={'flex flex-col items-center gap-2 p-3 rounded-xl text-center border ' + (logro.ok ? 'bg-[rgba(124,58,237,0.12)] border-[rgba(124,58,237,0.3)]' : 'bg-[rgba(124,58,237,0.06)] border-[rgba(124,58,237,0.15)] opacity-40')}>
                <span className="text-2xl">{logro.icon}</span>
                <span className="text-xs text-[#9CA3AF] font-medium leading-tight">{logro.label}</span>
              </div>
            ))}
          </div>
        </div>
