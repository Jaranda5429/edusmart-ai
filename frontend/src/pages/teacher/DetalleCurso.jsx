...
const DetalleCurso = () => {
  ...
  return (
    <div className="min-h-screen bg-grisSuave flex">
      ...
      {/* Sidebar */}
      <div className="w-64 bg-[#1C1535] flex flex-col border-r border-[rgba(124,58,237,0.15)]">
        ...
        <nav className="flex-1 p-4 space-y-1">
          {[
            ...
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-[#9CA3AF] hover:bg-lavanda hover:text-purple-700 transition-all text-sm font-medium"
            >
              <span>{item.icon}</span>
        ...
        <div style={{ background: "#1C1535" }} className="shadow-sm px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              ...
              className="text-[rgba(156,163,175,0.5)] hover:text-[#9CA3AF]"
            >
              ←
            </button>
            <div>
              <h2 className="text-xl font-bold text-[#E5E7EB]">{curso?.titulo}</h2>
              <p className="text-[rgba(156,163,175,0.7)] text-sm">
        ...
        <div className="flex-1 p-8">
            ...
            {['tareas', 'quizzes'].map((t) => (
              <button
                ...
                className={`px-5 py-2 rounded-xl text-sm font-medium transition-all capitalize ${
                  tab === t ? 'bg-purple-600 text-white' : 'bg-[rgba(124,58,237,0.08)] text-[rgba(167,139,250,0.7)] hover:bg-[rgba(124,58,237,0.15)]'
                }`}
              >
          ...
          {tab === 'tareas' && (
            <div className="space-y-4">
              {tareas.length === 0 ? (
                <div className="text-center py-16 text-[rgba(156,163,175,0.5)] bg-[#1C1535] rounded-2xl">
                  <div className="text-4xl mb-2">📝</div>
                  <p>No hay tareas aún</p>
                </div>
              ) : (
                tareas.map((tarea) => (
                  <div key={tarea.id} style={{ background: "#1C1535", borderRadius: 16, border: "1px solid rgba(124,58,237,0.18)" }} className="p-5 shadow-sm">
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        <h3 className="font-bold text-[#E5E7EB]">{tarea.titulo}</h3>
                        <p className="text-[rgba(156,163,175,0.7)] text-sm mt-1">{tarea.descripcion}</p>
                        <div className="flex gap-4 mt-2 text-xs text-[rgba(156,163,175,0.5)]">
                    ...
                    {tareaExpandida === tarea.id && (
                      <div className="border-t pt-3 mt-3">
                        {!tarea.entregas || tarea.entregas.length === 0 ? (
                          <p className="text-[rgba(156,163,175,0.5)] text-sm text-center py-4">
                            No hay entregas aún
                          </p>
                        ) : (
                                    ...
                                    {entrega.calificacion ? (
                                      ...
                                      <div className="flex items-center gap-2">
                                        <input
                                          ...
                                          max="10"
                                          placeholder="Nota"
                                          className="w-16 border border-[rgba(124,58,237,0.2)] rounded-lg px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-purple-400"
                                          onKeyDown={(e) => {
                                            if (e.key === 'Enter') {
                                        ...
                                        />
                                        <span className="text-xs text-[rgba(156,163,175,0.5)]">Enter para calificar</span>
                                      </div>
                                    )}
                                  </div>
                                </div>
                                <div style={{ background: "#1C1535", borderRadius: 10 }} className=" p-3">
                                  <p className="text-sm text-[#9CA3AF]">{entrega.contenido}</p>
                                  {entrega.archivoUrl && (
                                    <a
          ...
          {tab === 'quizzes' && (
            <div className="space-y-4">
              {quizzes.length === 0 ? (
                ...
                quizzes.map((quiz) => (
                  <div key={quiz.id} style={{ background: "#1C1535", borderRadius: 16, border: "1px solid rgba(124,58,237,0.18)" }} className="p-5 shadow-sm">
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        <h3 className="font-bold text-[#E5E7EB]">{quiz.titulo}</h3>
                              ...
                              <div key={resultado.id} className="bg-grisSuave rounded-xl p-4">
                                ...
                                <div className="space-y-2">
                                  {resultado.respuestas.map((r, i) => (
                                    ...
                                    >
                                      <p className="font-medium text-[#D1D5DB]">{r.pregunta}</p>
      ...
      {showModalTarea && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div style={{ background: "#1C1535", borderRadius: 16, border: "1px solid rgba(124,58,237,0.18)" }} className="p-6 w-full max-w-md shadow-xl">
            <h3 className="font-bold text-[#E5E7EB] text-lg mb-4">Crear Tarea</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-[#D1D5DB] mb-1">Título</label>
                <input
                  type="text"
                  value={formTarea.titulo}
                  onChange={(e) => setFormTarea({ ...formTarea, titulo: e.target.value })}
                  placeholder="Ej: Ensayo sobre la Revolución"
                  className="w-full border border-[rgba(124,58,237,0.2)] rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-[#D1D5DB] mb-1">Descripción</label>
                <textarea
                  value={formTarea.descripcion}
                  onChange={(e) => setFormTarea({ ...formTarea, descripcion: e.target.value })}
                  placeholder="Describe la tarea..."
                  className="w-full border border-[rgba(124,58,237,0.2)] rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 resize-none"
                  rows={3}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-[#D1D5DB] mb-1">Fecha límite</label>
                <input
                  type="date"
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                className="flex-1 border border-[rgba(124,58,237,0.2)] text-[#9CA3AF] py-3 rounded-xl text-sm font-medium hover:bg-grisSuave"
              >
                Cancelar
              </button>
              <button
      ...
      {showModalQuiz && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div style={{ background: "#1C1535", borderRadius: 16, border: "1px solid rgba(124,58,237,0.18)" }} className="p-6 w-full max-w-lg shadow-xl my-8">
            <h3 className="font-bold text-[#E5E7EB] text-lg mb-4">Crear Quiz</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-[#D1D5DB] mb-1">Título del Quiz</label>
                <input
                  type="text"
              ...
              {formQuiz.preguntas.map((pregunta, index) => (
                <div key={index} className="bg-grisSuave rounded-xl p-4">
                  <p className="text-sm font-medium text-[#D1D5DB] mb-2">Pregunta {index + 1}</p>
                  <input
                    type="text"
                    value={pregunta.texto}
                    onChange={(e) => actualizarPregunta(index, 'texto', e.target.value)}
                    placeholder="Escribe la pregunta..."
                    className="w-full border border-[rgba(124,58,237,0.2)] rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 mb-3 text-white"
                  />
                  <div className="space-y-2">
                    {pregunta.opciones.map((opcion, opIndex) => (
                      <div key={opIndex} className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          className="flex-1 rounded-lg px-3 py-2 text-sm focus:outline-none text-[#E5E7EB]" style={{ background: "rgba(255,255,255,0.06)", border: "1.5px solid rgba(124,58,237,0.3)" }}"
                        />
                      </div>
                    ))}
                  </div>
                  <p className="text-xs text-[rgba(156,163,175,0.5)] mt-2">Selecciona el radio de la respuesta correcta</p>
                </div>
              ))}
              <button
                onClick={agregarPregunta}
                className="w-full border-2 border-dashed border-[rgba(124,58,237,0.2)] text-[rgba(156,163,175,0.7)] py-3 rounded-xl text-sm hover:border-purple-400 hover:text-purple-600 transition-all"
              >
                + Agregar pregunta
              </button>
            </div>
