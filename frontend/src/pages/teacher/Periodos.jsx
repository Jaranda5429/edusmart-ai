  ...
  return (
    <div className="min-h-screen bg-[#F5F6FA] flex" style={{ fontFamily: 'Poppins, sans-serif' }}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="bg-white shadow-sm px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            {vista !== 'periodos' && (
              <button onClick={handleVolver} className="w-8 h-8 bg-[#F5F6FA] rounded-xl flex items-center justify-center text-gray-500 hover:bg-[#EDE7FF] hover:text-purple-600 transition-all">←</button>
            )}
            <div>
              <h2 className="text-xl font-bold text-gray-800">{tituloVista[vista]}</h2>
              <p className="text-gray-500 text-sm">{subtituloVista[vista]}</p>
            </div>
          </div>
        ...
        <div className="flex-1 p-8 overflow-auto">
            ...
            <div className="space-y-6">
              ...
              {grados.map(g => (
                <button key={g.id} onClick={() => { setGradoSel(g); setVista('materias') }}
                  className="bg-white rounded-2xl p-8 text-left hover:scale-105 transition-all shadow-sm border-2 border-gray-100 hover:border-purple-200 group">
                  <div className="text-5xl mb-4">{g.icon}</div>
                  <h3 className="text-2xl font-bold text-gray-800 mb-1">{g.nombre}</h3>
                  <p className="text-gray-400 text-sm">{g.materias.length} materia{g.materias.length > 1 ? 's' : ''} · {(estudiantes[g.id] || []).length} estudiantes</p>
            ...
            <div className="space-y-6">
              ...
              {gradoSel?.materias.length === 0 ? (
                <div className="bg-white rounded-2xl p-12 text-center shadow-sm border border-gray-100">
                  ...
                  <p className="text-gray-500 mt-3 font-medium">Sin materias aún</p>
                  ...
                  return (
                    <div key={m.id} className={`${c.bg} border-2 ${c.border} rounded-2xl p-6 shadow-sm`}>
                      <button onClick={() => { setMateriaSel(m); setVista('actividades') }} className="w-full text-left group">
                        ...
                        <p className="text-gray-500 text-sm">{count} actividades en {periodoSel?.nombre}</p>
                      ...
                      <div className="mt-4 pt-4 border-t border-white border-opacity-60">
                        {clave ? (
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="text-xs text-gray-500 mb-0.5">Clave de matrícula</p>
                            ...
                            <button
                              ...
                              className="text-xs bg-white bg-opacity-70 border border-gray-200 text-gray-600 px-3 py-1.5 rounded-lg hover:bg-white transition-colors">
                              ✏️ Cambiar
                            </button>
                          </div>
                        ) : (
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-3 flex-wrap">
                ...
                <button onClick={() => setMostrarClave(true)}
                  className="bg-white border-2 border-purple-200 text-purple-700 px-6 py-3 rounded-xl font-medium hover:bg-[#EDE7FF] transition-colors flex items-center gap-2">
                  <span>🔑</span>
              ...
              <div>
                ...
                {estudiantesGrado.length === 0 ? (
                  <div className="bg-white rounded-2xl p-10 text-center shadow-sm">
                    ...
                    <p className="text-gray-400 mt-2 text-sm">Sin estudiantes. Comparte la clave de matrícula o agrégalos desde Estudiantes.</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {estudiantesGrado.map(est => {
                      ...
                      return (
                        <button key={est.id} onClick={() => { setEstudianteSel(est); setVista('estudiante') }}
                          className="w-full bg-white rounded-2xl p-5 shadow-sm hover:shadow-md border-2 border-transparent hover:border-purple-200 transition-all text-left flex items-center gap-4 group">
          ...
          {vista === 'actividad_detalle' && actividadActual && (
            <div className="space-y-6">
              ...
              <div className="flex gap-2 flex-wrap">
                {['contenido', 'entregas', ...(actividadActual.foro ? ['foro'] : [])].map(tab => (
                  <button key={tab} onClick={() => setTabActiva(tab)}
                    className={`px-5 py-2.5 rounded-xl font-medium text-sm transition-all ${tabActiva === tab ? 'bg-purple-600 text-white' : 'bg-white text-gray-600 hover:bg-[#EDE7FF] hover:text-purple-700'}`}>
              ...
              {tabActiva === 'entregas' && (
                <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-100"><h3 className="font-bold text-gray-800">Entregas</h3></div>
                          ...
                          return (
                            <tr key={i} className="border-b border-gray-50 hover:bg-[#F5F6FA] transition-colors">
                              ...
                              <td className="py-4 px-6 text-center">
                                ...
                                ) : <span className="text-xs text-gray-400 bg-[#F5F6FA] px-3 py-1 rounded-full">Pendiente por calificar</span>}
                              </td>
                              <td className="py-4 px-6 text-center">
                                {ent.entregado && (
              ...
              {tabActiva === 'foro' && actividadActual.foro && (
                <div className="space-y-4">
                  <div className="bg-white rounded-2xl p-6 shadow-sm">
                    ...
                    {actividadActual.foro.fechaLimite && <p className="text-gray-400 text-xs">⏰ Fecha límite: {actividadActual.foro.fechaLimite}</p>}
                  </div>
                  <div className="space-y-3">
                        ...
                        <button key={i} onClick={() => { setForoEstSel({ ...est, resp }); setVista('foro_estudiante') }}
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div className="flex-1">
                              ...
                              {resp.respuesta ? <p className="text-gray-500 text-xs mt-0.5 truncate">{resp.respuesta}</p> : <p className="text-gray-400 text-xs italic">Sin respuesta aún</p>}
                            </div>
                            <div className="text-right">
                              {resp.fechaHora && <p className="text-xs text-gray-400 mb-1">{fmt(resp.fechaHora)}</p>}
          ...
          {vista === 'estudiante' && estudianteSel && (
            <div className="space-y-6">
              <div className="bg-white rounded-2xl p-6 shadow-sm flex items-center gap-6">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-800">{estudianteSel.nombre}</h3>
                  <p className="text-gray-400">{estudianteSel.email}</p>
              ...
              <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100"><h3 className="font-bold text-gray-800">Actividades asignadas</h3></div>
                {actividadesActuales.length === 0 ? (
                  <div className="p-8 text-center text-gray-400">Sin actividades</div>
                ) : (
                  <table className="w-full">
                    <thead><tr className="bg-[#F5F6FA] border-b border-gray-100">
                      <th className="text-left py-3 px-6 text-xs font-semibold text-gray-500 uppercase">Actividad</th>
                      <th className="text-center py-3 px-6 text-xs font-semibold text-gray-500 uppercase">Fecha límite</th>
                      <th className="text-center py-3 px-6 text-xs font-semibold text-gray-500 uppercase">Estado</th>
                      <th className="text-center py-3 px-6 text-xs font-semibold text-gray-500 uppercase">Calificación</th>
                      <th className="text-center py-3 px-6 text-xs font-semibold text-gray-500 uppercase">Acción</th>
                    </tr></thead>
                      ...
                      {actividadesActuales.map(act => {
                        ...
                        return (
                          <tr key={act.id} className="border-b border-gray-50 hover:bg-[#F5F6FA] transition-colors">
                            <td className="py-4 px-6 text-sm font-medium text-gray-800">{act.titulo}</td>
                            <td className="py-4 px-6 text-center text-sm text-gray-500">{act.fechaLimite}</td>
                            <td className="py-4 px-6 text-center">
                              <span className={`text-xs px-3 py-1 rounded-full font-semibold ${ent?.entregado ? 'bg-[#DDF7E9] text-green-700' : 'bg-[#FFE4D6] text-red-700'}`}>
                                {ent?.entregado ? '✅ Entregado' : '❌ Pendiente'}
                              </span>
          ...
          {vista === 'calificar' && entregaSel && (
            <div className="max-w-2xl space-y-6">
              <div className="bg-white rounded-2xl p-6 shadow-sm">
                ...
                {entregaSel.texto && <div className="bg-[#F5F6FA] rounded-xl p-4 text-sm text-gray-700 mb-3">{entregaSel.texto}</div>}
                {entregaSel.archivos?.length > 0 ? (
                  <div className="space-y-2">{entregaSel.archivos.map((a, i) => (
                    <div key={i} className="flex items-center gap-3 bg-[#F5F6FA] rounded-xl p-3"><span className="text-xl">📎</span><span className="text-sm text-gray-700">{a.nombre}</span></div>
                  ))}</div>
          ...
          {vista === 'foro_estudiante' && foroEstSel && (
            <div className="max-w-2xl space-y-6">
              <div className="bg-white rounded-2xl p-6 shadow-sm flex items-center gap-4">
                ...
                <div><h3 className="font-bold text-gray-800">{foroEstSel.nombre}</h3><p className="text-gray-400 text-sm">{foroEstSel.email}</p></div>
              </div>
              {foroEstSel.resp?.respuesta ? (
                <div className="bg-white rounded-2xl p-6 shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-bold text-gray-800">Respuesta al foro</h3>
                    <span className="text-xs text-gray-400">{fmt(foroEstSel.resp.fechaHora)}</span>
                  </div>
                  <div className="bg-[#F5F6FA] rounded-xl p-4 text-sm text-gray-700">{foroEstSel.resp.respuesta}</div>
                </div>
              ) : (
                <div className="bg-white rounded-2xl p-6 shadow-sm text-center">
                  <span className="text-4xl">💬</span>
                  <p className="text-gray-500 mt-2">Este estudiante aún no ha respondido</p>
                </div>
              )}
              <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100"><h3 className="font-bold text-gray-800">Comentarios ({foroEstSel.resp?.comentarios?.length || 0})</h3></div>
                <div className="p-4 space-y-3">
                  {(!foroEstSel.resp?.comentarios || foroEstSel.resp.comentarios.length === 0) ? (
                    ...
                    <div key={i} className="bg-[#F5F6FA] rounded-xl p-4">
                      <div className="flex items-center justify-between mb-2">
                        <span className="font-semibold text-sm text-gray-800">{com.autorNombre}</span>
                        <span className="text-xs text-gray-400">{fmt(com.fechaHora)}</span>
                      </div>
                      <p className="text-gray-600 text-sm">{com.texto}</p>
                    </div>
                  ))}
                </div>
                {foroEstSel.resp?.respuesta && (
      ...
      {claveRapidaMateria && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 w-full max-w-sm shadow-2xl">
            <h3 className="font-bold text-gray-800 text-lg mb-1">🔑 Clave de matrícula</h3>
            <p className="text-gray-400 text-sm mb-5">{gradoSel?.nombre} · {claveRapidaMateria.nombre} · {periodoSel?.nombre}</p>
            <div className="mb-5">
              <label className="text-sm font-medium text-gray-700 mb-1 block">Clave (máx. 8 caracteres)</label>
              <input type="text" value={claveRapidaInput}
                ...
                className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-center text-2xl font-bold tracking-widest focus:outline-none focus:border-purple-400" />
              <p className="text-gray-400 text-xs mt-2 text-center">Los estudiantes usarán esta clave para matricularse</p>
            </div>
      ...
      {mostrarModalGrado && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 w-full max-w-sm shadow-2xl">
            <h3 className="font-bold text-gray-800 text-lg mb-1">🎒 Nuevo Grado</h3>
            <p className="text-gray-400 text-sm mb-5">{periodoSel?.nombre}</p>
            <div className="mb-5">
              <label className="text-sm font-medium text-gray-700 mb-1 block">Nombre del grado</label>
      ...
      {mostrarModalMateria && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 w-full max-w-sm shadow-2xl">
            <h3 className="font-bold text-gray-800 text-lg mb-1">📚 Nueva Materia</h3>
            <p className="text-gray-400 text-sm mb-5">{gradoSel?.nombre}</p>
            <div className="space-y-4 mb-5">
              <div>
                <label className="text-sm font-medium text-gray-700 mb-1 block">Nombre de la materia</label>
              ...
              <div>
                <label className="text-sm font-medium text-gray-700 mb-2 block">Ícono</label>
                <div className="grid grid-cols-6 gap-2">
                  {['📖', '🔍', '🔢', '🔬', '🌍', '🎨', '💻', '🏃', '🎵', '📐', '🧪', '📝'].map(ic => (
                    <button key={ic} onClick={() => setNuevaMateriaIcon(ic)}
                      className={`text-2xl p-2 rounded-xl transition-all border-2 ${nuevaMateriaIcon === ic ? 'border-purple-400 bg-purple-50' : 'border-transparent hover:border-gray-200'}`}>
                      {ic}
                    </button>
                  ))}
                </div>
      ...
      {mostrarClave && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 w-full max-w-sm shadow-2xl">
            <h3 className="font-bold text-gray-800 text-lg mb-1">🔑 Clave de matrícula</h3>
            <p className="text-gray-400 text-sm mb-2">{gradoSel?.nombre} · {materiaSel?.nombre} · {periodoSel?.nombre}</p>
            ...
            <div className="mb-4">
              <label className="text-sm font-medium text-gray-700 mb-1 block">Nueva clave (máx. 8 caracteres)</label>
              <input type="text" value={claveInput} onChange={e => setClaveInput(e.target.value.toUpperCase().slice(0, 8))}
                ...
                className="w-full border border-gray-200 rounded-xl px-4 py-3 text-center text-xl font-bold tracking-widest focus:outline-none focus:ring-2 focus:ring-purple-300" />
              <p className="text-gray-400 text-xs mt-1 text-center">Los estudiantes usarán esta clave para matricularse</p>
            </div>
            <div className="flex gap-3">
              <button onClick={() => { setMostrarClave(false); setClaveInput('') }} className="flex-1 border border-gray-200 text-gray-600 py-3 rounded-xl font-medium hover:bg-gray-50 transition-colors">Cancelar</button>
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              <div className="bg-[#F5F6FA] rounded-xl p-4">
                ...
                {nuevaAct.foroActivo && (
                  <div className="mt-3 space-y-3">
                    <div>
                      <label className="text-sm font-medium text-gray-700 mb-1 block">Tema del foro *</label>
                    ...
                    <div>
                      <label className="text-sm font-medium text-gray-700 mb-1 block">Fecha límite del foro</label>
              ...
              {nuevaAct.contenidos.length > 0 && (
                <div>
                  <label className="text-sm font-medium text-gray-700 mb-2 block">Contenidos agregados ({nuevaAct.contenidos.length})</label>
                  ...
                  <div className="bg-[#F5F6FA] rounded-xl p-4 space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => { setTipoSel(null); setContenidoTemp({ texto: '', url: '', archivo: null }); setPreguntas([]) }}
                        className="ml-auto text-gray-400 hover:text-gray-600 text-sm">✕ Cancelar</button>
                    </div>
