...
const Actividades = () => {
  ...
  const Card = ({ children, className = '' }) => (
    <div className={`bg-[#1C1535] rounded-2xl border border-[rgba(124,58,237,0.18)] ${className}`}>{children}</div>
  )
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex" style={{ fontFamily: 'Poppins, sans-serif' }}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Header */}
        <div style={{ background: "#1C1535" }} className="border-b border-[rgba(124,58,237,0.2)] px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            {vista !== 'periodos' && (
              <button onClick={handleVolver} className="w-9 h-9 bg-gray-100 rounded-xl flex items-center justify-center text-[rgba(156,163,175,0.7)] hover:bg-purple-100 hover:text-purple-600 transition-all border border-[rgba(124,58,237,0.2)]">←</button>
            )}
            <div>
              ...
              <p className="text-[#E5E7EB] text-base">
            ...
            <div className="grid grid-cols-3 gap-6">
              {grados.map(g => (
                <button key={g.id} onClick={() => { setGradoSel(g); setVista('materias') }}
                  style={{ background: "#1C1535", borderRadius: 16, border: "1px solid rgba(124,58,237,0.18)" }} className="p-8 text-left hover:scale-105 transition-all shadow-sm border-2 border-[rgba(124,58,237,0.15)] hover:border-purple-300 group">
                  <div className="text-5xl mb-4">{g.icon}</div>
                  <h3 className="text-2xl font-bold text-[#E5E7EB] mb-1">{g.nombre}</h3>
                  <p className="text-[rgba(156,163,175,0.5)] text-sm">{g.materias.length} materia{g.materias.length > 1 ? 's' : ''}</p>
              ...
              {gradoSel?.materias.map((m, i) => {
                ...
                return (
                  <button key={m.id} onClick={() => { setMateriaSel(m); setVista('actividades') }}
                    ...
                    <p className="text-[rgba(156,163,175,0.7)] text-sm">{count} actividades</p>
                  ...
                  {actividadesActuales.map(act => {
                    ...
                    return (
                      <button key={act.id} onClick={() => { setActividadSel(act); setTabActiva('contenido'); setVista('actividad_detalle') }}
                        className="w-full bg-[#1C1535] rounded-2xl p-5 border border-[rgba(124,58,237,0.2)] hover:border-[rgba(124,58,237,0.5)] transition-all text-left group">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="w-full bg-gray-100 rounded-full h-1.5 mt-2 border border-[rgba(124,58,237,0.2)]">
              ...
              <div>
                ...
                {estudiantesGrado.length === 0 ? (
                  <Card className="p-10 text-center">
                    ...
                    <p className="text-[rgba(156,163,175,0.5)] mt-2 text-sm">Sin estudiantes en este grado</p>
                  </Card>
                ) : (
                  <div className="space-y-3">
                    {estudiantesGrado.map(est => {
                      ...
                      return (
                        <div key={est.id} style={{ background: "#1C1535", borderRadius: 16, border: "1px solid rgba(124,58,237,0.18)" }} className="p-4 shadow-sm border border-[rgba(124,58,237,0.15)] flex items-center gap-4">
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-2 flex-wrap">
                {['contenido', 'entregas', ...(actividadActual.foro ? ['foro'] : [])].map(tab => (
                  <button key={tab} onClick={() => setTabActiva(tab)}
                    className={`px-5 py-2.5 rounded-xl font-medium text-sm transition-all border ${tabActiva === tab ? 'bg-purple-600 text-white border-purple-600 shadow-md' : 'bg-[rgba(124,58,237,0.08)] text-[rgba(167,139,250,0.7)] border-[rgba(124,58,237,0.2)] hover:border-[rgba(124,58,237,0.5)] hover:text-[#A78BFA]'}`}>
                  ...
                  {(!actividadActual.contenidos || actividadActual.contenidos.length === 0) ? (
                    ...
                    <div className="p-4 space-y-3">
                      {actividadActual.contenidos.map((c, i) => (
                        <div key={i} className="flex items-start gap-4 p-4 bg-gray-50 rounded-xl border border-[rgba(124,58,237,0.15)]">
                          ...
                          <div className="flex-1">
                            ...
                            {c.preguntas && (
                              <div className="mt-2 space-y-2">
                                {c.preguntas.map((p, j) => (
                                  <div key={j} style={{ background: "#1C1535", borderRadius: 10 }} className=" p-3 border border-[rgba(124,58,237,0.15)]">
              ...
              {tabActiva === 'entregas' && (
                <Card className="overflow-hidden">
                  <div className="px-6 py-4 border-b border-[rgba(124,58,237,0.15)]"><h3 className="font-bold text-[#E5E7EB]">Entregas de estudiantes</h3></div>
                    ...
                    <table className="w-full">
                      ...
                      <tbody>
                        {actividadActual.entregas.map((ent, i) => {
                          ...
                          if (!est) return null
                          return (
                            <tr key={i} className="border-b border-gray-50 hover:bg-[rgba(124,58,237,0.1)] transition-colors">
                              ...
                              <td className="py-4 px-6 text-center">
                                ...
                                ) : <span className="text-xs text-[rgba(156,163,175,0.5)] bg-gray-100 px-3 py-1 rounded-full border border-[rgba(124,58,237,0.2)]">Pendiente por calificar</span>}
                              </td>
                              <td className="py-4 px-6 text-center">
                                {ent.entregado && (
              ...
              {tabActiva === 'foro' && actividadActual.foro && (
                <Card className="p-6">
                  ...
                  {actividadActual.foro.fechaLimite && <p className="text-[rgba(156,163,175,0.5)] text-xs mb-4">⏰ Fecha límite: {actividadActual.foro.fechaLimite}</p>}
                  <div className="space-y-3">
                      ...
                      return (
                        <div key={i} style={{ background: "rgba(124,58,237,0.06)" }} className="rounded-xl p-4 border border-[rgba(124,58,237,0.15)]">
                          <div className="flex items-center gap-3 mb-2">
                            <div className="w-8 h-8 bg-purple-100 rounded-full flex items-center justify-center text-purple-600 font-bold text-sm border border-purple-200">{est.nombre.charAt(0)}</div>
                            <span className="font-bold text-[#E5E7EB] text-sm">{est.nombre}</span>
                            {resp.fechaHora && <span className="text-xs text-[rgba(156,163,175,0.5)] ml-auto">{fmt(resp.fechaHora)}</span>}
                          </div>
                          {resp.respuesta ? <p className="text-[#9CA3AF] text-sm">{resp.respuesta}</p> : <p className="text-[rgba(156,163,175,0.5)] text-xs italic">Sin respuesta aún</p>}
                        </div>
                      )
                    })}
                  </div>
          ...
          {vista === 'calificar' && entregaSel && (
            <div className="max-w-2xl space-y-6">
              <Card className="p-6">
                ...
                {entregaSel.texto && <div style={{ background: "rgba(124,58,237,0.06)" }} className="rounded-xl p-4 text-sm text-[#D1D5DB] mb-3 border border-[rgba(124,58,237,0.15)]">{entregaSel.texto}</div>}
      ...
      {mostrarModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div style={{ background: "#1C1535" }} className="rounded-2xl w-full max-w-2xl shadow-2xl max-h-[90vh] flex flex-col border border-[rgba(124,58,237,0.2)]">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              {nuevaAct.contenidos.length > 0 && (
                <div>
                  <label className="text-sm font-medium text-[#D1D5DB] mb-2 block">Contenidos ({nuevaAct.contenidos.length})</label>
                  <div className="space-y-2">
                    {nuevaAct.contenidos.map((c, i) => (
                      <div key={i} className="flex items-center gap-3 bg-gray-50 rounded-xl p-3 border border-[rgba(124,58,237,0.15)]">
                        <span>{c.icon}</span>
              ...
              <div>
                ...
                {!tipoSel ? (
                  <div className="grid grid-cols-3 gap-2">
                    {TIPOS_CONTENIDO.map(t => (
                      <button key={t.id} onClick={() => setTipoSel(t)}
                        style={{ background: "rgba(124,58,237,0.06)" }} className="hover:bg-purple-50 rounded-xl p-3 text-center transition-colors border border-[rgba(124,58,237,0.2)] hover:border-purple-300">
                  ...
                  <div style={{ background: "rgba(124,58,237,0.06)" }} className="rounded-xl p-4 space-y-3 border border-[rgba(124,58,237,0.2)]">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => { setTipoSel(null); setContenidoTemp({ texto: '', url: '', archivo: null }); setPreguntas([]) }}
                        className="ml-auto text-[rgba(156,163,175,0.5)] hover:text-[#9CA3AF] text-sm">✕</button>
                    </div>
                    ...
                    {tipoSel.id === 'quiz' && (
                      <div className="space-y-3">
                        {preguntas.length > 0 && (
                          <div className="space-y-2">
                            {preguntas.map((p, i) => (
                              <div key={i} style={{ background: "#1C1535", borderRadius: 10 }} className=" p-3 border border-[rgba(124,58,237,0.15)] text-sm">
                        ...
                        <div style={{ background: "#1C1535", borderRadius: 12, border: "1px solid rgba(124,58,237,0.18)" }} className="p-4 space-y-2 border border-[rgba(124,58,237,0.15)]">
